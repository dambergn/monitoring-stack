...
if [ ! -f "/etc/certs/localhost.key" ] && [ ! -f "/etc/certs/localhost.crt" ]; then
    echo "Generating new certificates..."
    # Generate Self-Signed Certificates
    openssl req -x509 -newkey rsa:4096 -keyout /etc/certs/localhost.key -out /etc/certs/localhost.crt -days 365 -nodes -subj "/C=US/ST=State/L=Locality/O=Organization/CN=localhost"
else
